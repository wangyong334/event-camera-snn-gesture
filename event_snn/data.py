...
SNN 是按时间步 (time step) 一步步处理输入的，所以我们要把一段连续的
...
“事件 -> 帧”的积分由 SpikingJelly 的 data_type='frame' 自动完成，不用自己写积分循环。
"""

#调用 SpikingJelly 框架(它内置了 DVS128 Gesture 的处理)
...
def get_dvs128_loaders(root, T=16, batch_size=8, num_workers=4):
    ...
    参数：
        root        : 数据集根目录 (里面有 download/DvsGesture.tar.gz)
        T           : 时间步数，也就是把事件切成几片 (= SNN 的仿真步数)
        batch_size  : 批大小。My PC-8GB 显存，建议 8；若显存不足改 4
