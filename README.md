...
| 测试准确率 | **78.82%（64 epoch）/88.89%（128 epoch）**（227/288 量级，11 分类） |
...
数据集下载链接：https://ibm.ent.box.com/s/3hiq58ww1pbbjrinh367ykfdf60xsfm8/
...
2. **前向传播（多步模式）**：输入沿 T 步推进，各 PLIF 神经元逐步积分膜电位、过阈值发放脉冲。
3. **输出**：对输出层 T 步脉冲取平均得发放率，作为各类得分。
4. **损失与反向传播**：交叉熵损失；BPTT 沿时间+层回传，发放处用替代梯度。
...
- **准确率**：测试集 78.82%（64 epoch）/88.89%（128 epoch）（11 分类，受试者无重叠划分）。
